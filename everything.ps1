...
function Write-DoubleBoxTop    { if ($Quiet) { return }; Write-Host ("  $($script:UI.TL)" + ($script:UI.H.ToString() * ($script:UI.Width + 2)) + "$($script:UI.TR)") -ForegroundColor DarkRed }
function Write-DoubleBoxBottom { if ($Quiet) { return }; Write-Host ("  $($script:UI.BL)" + ($script:UI.H.ToString() * ($script:UI.Width + 2)) + "$($script:UI.BR)") -ForegroundColor DarkRed }
function Write-DoubleBoxDivider { if ($Quiet) { return }; Write-Host ("  $($script:UI.ML)" + ($script:UI.H.ToString() * ($script:UI.Width + 2)) + "$($script:UI.MR)") -ForegroundColor DarkRed }
...
function Invoke-MemoryStringScanPhase {
    ...
    if ($SkipMemoryScan) {
        Write-Host "   Skipped (-SkipMemoryScan specified)." -ForegroundColor DarkGray
        return @{}
    }
...
function Invoke-AntiForensicsPhase {
    Write-PhaseHeader -Text ("PHASE 12 / {0}  -  ANTI-FORENSICS / TAMPER-EVIDENCE CHECK" -f $script:Config.TotalPhases)
    ...
    $eventChecks = @(
        ...
        @{ Log = "System";      IDs = @(1074);       Label = "User-initiated shutdown/restart"; Severity = "INFO" },
        @{ Log = "System";      IDs = @(6005);       Label = "Event Log service (re)started"; Severity = "INFO" }
    )

    foreach ($check in $eventChecks) {
        try {
            $filter = "*[System[EventID=$($check.IDs -join ' or EventID=')]]"
            $event = Get-WinEvent -LogName $check.Log -FilterXPath $filter -MaxEvents 1 -ErrorAction SilentlyContinue
            if ($event) {
                $eventTime = $event.TimeCreated.ToString("yyyy-MM-dd HH:mm:ss")
                Write-Host ("   {0} (Event ID {1}) - last seen: {2}" -f $check.Label, $event.Id, $eventTime) -ForegroundColor $(if ($check.Severity -eq "INFO") { "Gray" } else { "Yellow" })
                if ($check.Severity -ne "INFO") {
                    $antiForensicHits++
                    Add-Finding $check.Severity "AntiForensics" ("$($check.Label) detected (Event ID $($event.Id) at $eventTime)")
                }
            } else {
                Write-Host ("   {0}: not found" -f $check.Label) -ForegroundColor DarkGray
            }
        } catch {
            Write-Host ("   {0}: unable to query log" -f $check.Label) -ForegroundColor DarkGray
        }
    }
    ...
    try {
        ...
        if (Test-Path -Path $recycleBinFolderPath) {
            $recycleBinFolder = Get-Item -Path $recycleBinFolderPath -Force -ErrorAction SilentlyContinue
            if ($recycleBinFolder) {
                $lastModifiedTime = $recycleBinFolder.LastWriteTime
                Write-Host ("   Recycle Bin last modified: {0}" -f $lastModifiedTime.ToString("yyyy-MM-dd HH:mm:ss")) -ForegroundColor Gray

                $mcStart = Get-MinecraftStartTime
                if ($mcStart -and $lastModifiedTime -gt $mcStart) {
                    $antiForensicHits++
                    Add-Finding "INFO" "AntiForensics" ("Recycle Bin was modified after Minecraft started ($($lastModifiedTime.ToString('yyyy-MM-dd HH:mm:ss'))) - files may have been deleted during play")
                }
            }
        } else {
            Write-Host "   Recycle Bin folder for current user not found." -ForegroundColor DarkGray
        }
    } catch {
    ...
    try {
        ...
        if ($null -ne $prefetchStatus) {
            if ($prefetchStatus -gt 0) {
                Write-Host "   Prefetching: Enabled" -ForegroundColor Green
            } else {
                Write-Host "   Prefetching: DISABLED" -ForegroundColor Red
                $antiForensicHits++
                Add-Finding "MEDIUM" "AntiForensics" "Windows Prefetch is disabled - this destroys evidence of which programs have been run"
            }
        } else {
            Write-Host "   Prefetching: unable to read setting" -ForegroundColor DarkGray
        }
    } catch {
        Write-Host "   Prefetching: unable to read setting" -ForegroundColor DarkGray
    }
    ...
    if (Test-Path $prefetchPath) {
        $hiddenFiles = Get-ChildItem -Path $prefetchPath -Force -ErrorAction SilentlyContinue | Where-Object { $_.Attributes -match "Hidden" }
        if ($hiddenFiles) {
            Write-Host ("   {0} hidden file(s) found in Prefetch folder" -f $hiddenFiles.Count) -ForegroundColor Red
            $antiForensicHits++
            Add-Finding "HIGH" "AntiForensics" ("$($hiddenFiles.Count) hidden file(s) found in Prefetch folder - possible attempt to conceal execution evidence")
        } else {
            Write-Host "   No hidden files found in Prefetch." -ForegroundColor Green
        }

        $readOnlyFiles = Get-ChildItem -Path $prefetchPath -Force -ErrorAction SilentlyContinue | Where-Object { $_.Attributes -match "ReadOnly" }
        if ($readOnlyFiles) {
            Write-Host ("   {0} read-only file(s) found in Prefetch folder" -f $readOnlyFiles.Count) -ForegroundColor Yellow
            $antiForensicHits++
            Add-Finding "MEDIUM" "AntiForensics" ("$($readOnlyFiles.Count) read-only file(s) found in Prefetch folder - unusual attribute change")
        } else {
    ...
    $criticalServices = @(
        ...
        @{ Name = "DusmSvc";    Label = "Data Usage / Diagnostic Service" },
        @{ Name = "EventLog";   Label = "Windows Event Log" },
        @{ Name = "AppInfo";    Label = "Application Information" },
        @{ Name = "DcomLaunch"; Label = "DCOM Server Process Launcher" }
    )

    foreach ($svcDef in $criticalServices) {
        $svc = Get-Service -Name $svcDef.Name -ErrorAction SilentlyContinue
        if (-not $svc) {
            Write-Host ("   {0}: not found on this system" -f $svcDef.Label) -ForegroundColor DarkGray
            continue
        }
        if ($svc.Status -eq 'Running') {
            Write-Host ("   {0}: Running" -f $svcDef.Label) -ForegroundColor Green
        } else {
            Write-Host ("   {0}: {1}" -f $svcDef.Label, $svc.Status) -ForegroundColor Yellow
            # EventLog stopped is far more suspicious than the others being stopped (which can be normal on some systems)
            if ($svcDef.Name -eq "EventLog") {
                $antiForensicHits++
                Add-Finding "HIGH" "AntiForensics" "Windows Event Log service is not running - system-wide logging is currently disabled"
            } elseif ($svcDef.Name -in @("SysMain", "DPS", "PcaSvc")) {
                $antiForensicHits++
                Add-Finding "INFO" "AntiForensics" ("$($svcDef.Label) is not running - reduces available diagnostic/usage history (often normal on tuned systems)")
            }
        }
    }

    if ($antiForensicHits -eq 0) {
        Write-Host "   [OK] No anti-forensic / tamper-evidence indicators found." -ForegroundColor Green
    }
}

function Export-JsonReport {
    param($Verified, $Unknown, $Threats, $MemoryResults)
    $path = Join-Path $env:USERPROFILE "Downloads\VampCheatScanner-Report-$(Get-Date -Format 'yyyyMMdd-HHmmss').json"
    $report = @{
...
"@
    ...
    $sysFindingsNoForensics = @($actionableFindings | Where-Object { $_.Module -ne "AntiForensics" })
    $sysHtml = ""
    if ($sysFindingsNoForensics.Count -gt 0) {
        $rows = ""
        foreach ($f in ($sysFindingsNoForensics | Sort-Object Severity)) {
...
"@
    ...
    <div class="panel">
      <div class="panel-header">
        ...
        <div class="badge badge-flagged">$($sysFindingsNoForensics.Count) FINDING(S)</div>
      </div>
      <div class="panel-body">$rows</div>
    </div>
"@
    ...
    $forensicsFindings = @($script:Findings | Where-Object { $_.Module -eq "AntiForensics" })
    $forensicsActionable = @($forensicsFindings | Where-Object { $_.Severity -ne "INFO" })
    $forensicsHtml = ""
    if ($forensicsFindings.Count -gt 0) {
        $rows = ""
        foreach ($f in ($forensicsFindings | Sort-Object Severity)) {
        ...
        <div class="term-row">
          <div class="term-head">
            <span class="term-name">$($f.Severity)</span>
            <span class="badge $badgeClass">$($f.Severity)</span>
          </div>
          <div class="detail-line">$(HtmlEncode $f.Detail)</div>
        </div>
"@
        ...
        $forensicsBadgeClass = if ($forensicsActionable.Count -gt 0) { "badge-flagged" } else { "badge-clean" }
        $forensicsBadgeText  = if ($forensicsActionable.Count -gt 0) { "$($forensicsActionable.Count) FINDING(S)" } else { "CLEAN" }
        $forensicsHtml = @"
    <div class="panel">
      <div class="panel-header">
        <div><div class="panel-title">Anti-Forensics / Tamper Evidence</div><div class="panel-sub">Event log clears, prefetch tampering, disabled logging services</div></div>
        <div class="badge $forensicsBadgeClass">$forensicsBadgeText</div>
      </div>
      <div class="panel-body">$rows</div>
    </div>
"@
    ...
    $infoFindingsNoForensics = @($infoFindings | Where-Object { $_.Module -ne "AntiForensics" })
    $infoHtml = ""
    if ($infoFindingsNoForensics.Count -gt 0) {
        $rows = ""
        foreach ($f in $infoFindingsNoForensics) {
    ...
    <div class="panel">
      <div class="panel-header">
        ...
        <div class="badge badge-info">$($infoFindingsNoForensics.Count) NOTE(S)</div>
      </div>
      <div class="panel-body">$rows</div>
    </div>
"@
  ...
  <div class="container">
    <div class="header">
      <div>
        ...
        <div class="meta">Version $($script:Config.Version) &middot; jar verification + system checks + anti-forensics + live memory scan</div>
      </div>
    ...
    <div class="stats">
      ...
      <div class="stat"><div class="value">$($forensicsActionable.Count)</div><div class="label">Tamper Indicators</div></div>
    </div>
...
function Main {
    ...
    $reportPath = Join-Path $env:USERPROFILE "Downloads\VampCheatScanner-Report-$(Get-Date -Format 'yyyyMMdd-HHmmss').html"
